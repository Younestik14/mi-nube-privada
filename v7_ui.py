...
def overview():
    ...
    st.markdown('<div class="section">Distribución de cargas</div>',unsafe_allow_html=True);ph,imb=phase_balance(cs);x,y,z=st.columns(3);x.metric('L1',f'{ph[1]:.1f} kW');y.metric('L2',f'{ph[2]:.1f} kW');z.metric('L3',f'{ph[3]:.1f} kW');st.caption(f'Desequilibrio relativo estimado: {imb:.1f}%')
    if cs:
