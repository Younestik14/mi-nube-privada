...
st.markdown(
    ...
    /* Ajuste de labels y textos de Streamlit a blanco */
    ...
    /* Estética de Resultados Técnicos (Ahora optimizada para fondo negro) */
    .resultado-caja {
        color: #ffffff !important; 
        ...
        padding: 15px; 
        border-radius: 10px;
        border-left: 6px solid #22d3ee; /* Cyan eléctrico */
        margin-bottom: 10px; 
        text-align: right;
        box-shadow: 0px 4px 10px rgba(0,0,0,0.5);
    }

    /* Banner de Total Final (Gold Edition) */
    .total-final-banner {
        color: #ffffff !important; 
        font-weight: 900 !important; 
        font-size: 38px;
    ...
    /* Estilo de los Expanders en Dark Mode */
    .stExpander { 
...
# --- 2. MOTOR DE CÁLCULO REBT (ITC-BT-19) ---
...
if modo == "📐 Calculadora REBT":
    ...
    with c2:
        ...
        metodo_inst = st.selectbox("Instalación", list(tablas_adm.keys()) + ["D1 - Enterrado"])
    ...
    st.divider()
    st.markdown(f'<div class="resultado-caja">SECCIÓN CALCULADA: {s_final} mm² <br><small>Ib: {ib:.2f} A | CdT: {s_cdt:.2f} mm²</small></div>', unsafe_allow_html=True)
