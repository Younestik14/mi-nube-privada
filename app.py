...
# --- 1. CONFIGURACIÓN Y MARCA DE AGUA CENTRADA ---
...
if menu == "📐 Calculadora REBT":
    ...
    r1.metric("Intensidad Ib", f"{Ib:.2f} A")
    r2.metric("Sección por Voltaje", f"{S_cdt:.2f} mm²")
    if seccion_ok:
        r3.metric("SECCIÓN RECOMENDADA", f"{seccion_ok} mm²")
        st.success(f"Cable validado para {iz_actual:.2f} A (corregido).")
    else:
